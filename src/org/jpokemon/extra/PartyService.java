...
public class PartyService implements JPokemonService {
  ...
  public void serve(JSONObject request, Player player) throws ServiceException {
    try {
      ...
      if (method.equals("slide")) {
        slide(request, player);
      }
    }
  ...
  private void slide(JSONObject request, Player player) throws JSONException {
    int indexOld = request.getInt("indexOld");
    int indexNew = request.getInt("indexNew");

    player.party().slide(indexOld, indexNew);
    PlayerManager.pushJson(player, load(null, player));
  }
